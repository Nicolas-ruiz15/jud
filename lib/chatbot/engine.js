...
export class UltraMasterJudaicaChatbot {
    ...
    try {
      ...
      try {
        this.components.productSearch = new UltraJudaicaProductSearch(
          ...
          this.updateDatabaseStatus.bind(this)
        );
  ...
  updateDatabaseStatus(status = {}) {
    const normalizedStatus = {
      connected: status.connected ?? this.databaseStatus.connected ?? null,
      checkedAt: status.checkedAt || new Date().toISOString(),
      latency: typeof status.latency === 'number' ? status.latency : null,
      error: status.error || null,
      source: status.source || 'engine'
    };

    this.databaseStatus = normalizedStatus;
  }
